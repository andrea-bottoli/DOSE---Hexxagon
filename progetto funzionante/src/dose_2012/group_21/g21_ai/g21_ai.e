...
feature{G21_AI} -- Attributes
	...
	board: ARRAY[ARRAY[G21_CELL]] -- a link to the board used by the Logic component
	cards: ARRAYED_LIST[G21_CARD] -- a link to the AI deck stored in Logic component
	free_positions: ARRAYED_LIST[G21_POINT] -- a list of all the position available on the board during AI turn
...
feature{G21_AI, G21_BOARD} -- Interface Procedures, the only method the Logic component can use to call the AI
	...
	make_a_move (card_position: G21_POINT; player_card: G21_CARD): G21_MOVE --it returns and object move that the Logic will use to make AI move
		...
		deferred
			...
			free_position: board.item(result.position.x).item(result.position.y)=void
			...
			board_not_changed_first_row: board.item(1).item(1) = old board.item(1).item(1) and then board.item(1).item(2) = old board.item(1).item(2) and then board.item(1).item(3) = old board.item(1).item(3)
			board_not_changed_second_row: board.item(2).item(1) = old board.item(2).item(1) and then board.item(2).item(2) = old board.item(2).item(2) and then board.item(2).item(3) = old board.item(2).item(3)
			board_not_changed_third_row: board.item(3).item(1) = old board.item(3).item(1) and then board.item(3).item(2) = old board.item(3).item(2) and then board.item(3).item(3) = old board.item(3).item(3)
		end
		...
		do
			...
			loop
				...
				until
					row<=3
				loop
					if board.item(row)/=void and then board.item(row).item(column)/=void and then board.item(row).item(column).isOccupied=FALSE
					then
