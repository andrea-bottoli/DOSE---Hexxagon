...
--	end
	...
	connect(my_name: STRING; server_ip: STRING; server_port: INTEGER): BOOLEAN
		-- this feature ask to the NET component to enstrablish connection with the server indicated using server_ip and
		-- server_port and to inform him, that the player wold like to join a match using the name "my_name"
		require
			-- a valid server_ip
			valid_port: server_port > 1024 and server_port < 5000
			valid_arg: my_name /= void
			-- my_name isn't already in use. in this came the host inform the player that is name is invalid and close the
			-- connection, the player can try to re-connect using another name
		do
		ensure
			-- If all the require are satisfied and the return value si true (also the name is not in use) ensure that
			-- the connection with the server is corrected enstablish, and the player has joined the game.
			-- Otherwise the connection is not enstablish.
		end

	play_card(card: STRING)
	-- this is invoked by the GUI to inform which card the player want to play
		require
			valid_card_id:
		...
		do
			...
			-- this message will be send to the host
		end

	selected_from_supply(cards: ARRAY[STRING])
	-- this is invoked by the GUI to inform the net about cards chosen from a pop-up
		require
	...
	-- this feature is invoked when the player expresses his/her intention to leave the match
		...
		do
			...
			-- the server will be notified and the match can continue if the client was a host.
...
feature -- Status Report
	...
	number_of_players: INTEGER
		-- indicates the number of players that are playing the game.
