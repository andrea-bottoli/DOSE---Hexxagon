...
feature -- Test routines
	...
	test_terrain_panel_update_tile
			-- New test routine
		note
			testing:  "covers/{G10_CRSN_TERRAIN_PANEL}.update_tile"
		local
			a_terrain_panel:G10_CRSN_TERRAIN_PANEL
			a_adding_tile:G10_CRSN_TILE
			x,y,i:INTEGER
		do
			from
				i:=0
			...
			loop
					x := 1+i
					y := 1+i
					a_terrain_panel.update_tile (x, y, a_adding_tile)
	...
	note
		testing:  "covers/{G10_CRSN_TERRAIN_PANEL}.update_background_color"
	local
		a_terrain_panel:G10_CRSN_TERRAIN_PANEL
		a_color:EV_COLORIZABLE
	do
		a_terrain_panel.update_background_color (a_color)
		assert ("test_terrain_panel_update_background_color result is ",a_color.is_background_color_void())
	end

	--test for TERRAIN_PANEL::get_background_color method
	test_terrain_panel_get_background_color
	note
		testing:  "covers/{G10_CRSN_TERRAIN_PANEL}.get_background_color"
	local
		a_terrain_panel:G10_CRSN_TERRAIN_PANEL
	do
		assert ("test_terrain_panel_get_background_color results is ",a_terrain_panel.get_background_color.is_foreground_color_void())
	end

	--test for TERRAIN_PANEL::get_terrain method
	test_terrain_panel_get_terrain
	note
		testing:  "covers/{G10_CRSN_TERRAIN_PANEL}.get_terrain"
	local
		a_terrain_panel:G10_CRSN_TERRAIN_PANEL
	do
	...
	note
		testing:  "covers/{G10_CRSN_TERRAIN_PANEL}.make"
	local
		a_terrain_panel:G10_CRSN_TERRAIN_PANEL
	do
		--create a_terrain_panel
		!!a_terrain_panel.make()
