...
feature -- Test routines
	...
	test_deny_client_0
		...
		do
			create master_controller.make
			game_players := master_controller.game.players.count
			if master_controller.incoming_clients.has (name) then
				master_controller.deny_client (name)
				player_in_list := True
			end
			assert ("List of players accepted not change:", (game_players = master_controller.game.players.count) and player_in_list)
		end
	...
	test_accept_player
		...
		do
			...
			if (master_controller.incoming_clients.has (name)) and ((pawn >= 0) and (pawn <= 5)) and (connection /= Void) then
				master_controller.accept_player (name, pawn, connection)
			end
			assert ("The player was accepted to the game list:", (game_players + 1 = master_controller.game.players.count) and not master_controller.incoming_clients.has (name))
		end
	...
	test_start_game
		...
		do
			...
			assert ("The game was started", (master_controller.game.current_player = master_controller.game.players [0]) and master_controller.game.players.count <= 6)
		end
