...
feature -- Test routines
	...
	test_generate_solution
		...
		note
			...
			testing: "user/CU"
		local
			available_cards_deck: INTEGER
		do
			create game.make
			available_cards_deck := game.cards_deck.count
			if (game.current_status = game.constants.game_states.starting) then
				game.generate_solution
			end
			assert ("The current game generates a solution:", (game.cards_deck.count = available_cards_deck - 3) and (game.solution.count = 3))
		end

	test_distribute_cards
			-- Test used to distribute cards to the players of game
		note
			testing: "covers/{CU_GAME}.distribute_cards"
			testing: "user/CU"
		local
			index: INTEGER
			p_cards: BOOLEAN
		do
			...
			from
				index := 0
			until
				(index > 5) or (p_cards = False)
			loop
				if (game.players[index].hand.count < 3) then
					p_cards := False
				end
				index := index + 1
			end
			assert ("The cards was distributed to the players of game:", (game.cards_deck.count = 0) and p_cards)
		end
	...
	test_remove_player
		...
		do
			create game.make
			if (game.players.has (player)) then
				game.remove_player (player)
			end
			assert ("The player was removed from the game:", not game.players.has (player))
		end
	...
	test_throw_dices
		...
		do
			...
			if (game.current_status = game.constants.game_states.dices) then
				game.current_player.add_available_moves (game.throw_dices)
			end
