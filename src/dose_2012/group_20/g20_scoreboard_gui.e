note
	description: "Implementation of the Class {G20_SCOREBOARD_GUI}(#GUI) of the Game 'Ingenious' "
	author: "Louloudakis Nick - crete4"
	date: "12/12/2012"
	revision: "1.0.0"

class G20_SCOREBOARD_GUI
...
feature {ANY}
	...
	--methods to get the current step of a color piece
	getStep(colorArgument:INTEGER):INTEGER
	require
		colorArgument >= 1 and colorArgument <= 6
	local
		tmpInteger:REAL_64
	do
		tmpInteger :=  (positions[colorArgument] - 15)/9
		Result := tmpInteger.truncated_to_integer
	end

	getStepColor(color:G20_COLOR):INTEGER
	require
		color.getColor >= 1 and color.GetColor <= 6
	do
		Result := getStep(color.getcolor)
	end
