...
feature -- Test routines
	...
	test_get_piece
		...
		do
			...
			create l_possible_move.make_possible_moves
			l_piece := 10
			l_possible_move.set_move (l_piece, l_piece, 1)
			assert ("Possible move is created ", l_possible_move/=Void)
			...
			l_piece := 10
			l_position:= 15
			l_possible_move.set_move (l_piece, l_position, 2)
			assert ("Possible move is created ", l_possible_move/=Void)
			assert ("Check get piece", l_possible_move.get_piece.is_equal (l_piece))
		end
	...
	test_get_position
		...
		do
			...
			create l_possible_move.make_possible_moves
			l_position := 7
			l_possible_move.set_move (l_position, l_position, 1)
			assert ("Possible move is created ", l_possible_move/=Void)
	...
	test_get_type
		...
		do
			...
			l_type := 1
			l_position := 6
			l_possible_move.set_move (l_position, l_position, l_type)
			assert ("Possible move is created ", l_possible_move/=Void)
			...
			l_piece:=1
			l_position := 6
			l_possible_move.set_move (l_piece, l_position, l_type)
			assert ("Possible move is created ", l_possible_move/=Void)
			assert ("Check get type", l_possible_move.get_type.is_equal (l_type))
		end
